...
def merge_first_layer_res(nn_res, rf_res):
	'''
	...
	columns = ['NN', 'RF']
	train_df = get_merged_df([nn_res[0], rf_res[0]], columns)
	validation_df = get_merged_df([nn_res[1], rf_res[1]], columns)
	test_df = get_merged_df([nn_res[2], rf_res[2]], columns)
...
def run_top_xgb(nn_res, rf_res, train_labels, validation_labels, ids_test):
	'''
	...
	train_df, validation_df, test_df = merge_first_layer_res(nn_res, rf_res)
	...
	param = {'max_depth':1, 'eta':0.1, 'min_child_weight':1, 'silent':1, 'objective':'binary:logistic', 'eval_metric':'logloss', 'subsample':1, 'col_sample_bytree':0.6 }
...
if __name__ == '__main__':
	...
	run_top_xgb(nn_res, rf_res, train_labels, validation_labels, ids_test)
