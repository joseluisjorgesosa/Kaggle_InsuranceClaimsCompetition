...
def get_training_data(training_file): 
	...
	X_train[0:len(traindf),0,0:len(features)] = np.asarray(traindf.iloc[0:len(traindf),1:])
	...
	X_val[0:len(valdf),0,0:len(features)] = np.asarray(valdf.iloc[0:len(valdf),1:])	
...
def get_test_data(test_data_filename):
	...
	X_test[0:len(testdf),0,0:len(features)] = testdf.iloc[0:len(testdf),0:]
...
if __name__ == "__main__":
	...
	#Set paramters
	opts = {
		'net1': True,				#If true uses a neural net that is predetermined
		'customNet': False,			#If true uses a neural net specified by parameters below
		...
		'hidden_layers' : 1,		#Total number of hidden layers
		'hidden_units': 200,		#Hidden units in each layer
		'drop_input' : False,		#Randomly remove a certain proportion of input units
		'drop_hidden': False,		#Randomly remove a certain proportion of hidden units
